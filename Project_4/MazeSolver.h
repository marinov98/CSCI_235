...
class MazeSolver {
  public:
	...
	// post: if input file cannot be read outputs "Cannot read from input_file_name"
  ...
  private:
	...
	char** solution_ = nullptr; // a 2-d character array containing maze characters as in maze_ and
	                            // path to exit marked with '>' characters
	...
	// pre: maze_ has been allocated with the correct number of rows and columns read from input
	// file post: fills in maze_ with characters read from input file called by constructor
	...
	// pre: old_position is a Position initialized with row and column to valid positions in maze_
	// and it is extensible in direction dir return: a new Position on the maze moving in direction
	// dir from old_position called by extendPath()
